...
describe 'force merge of file' do
  ...
  describe 'when run should not force' do
    ...
    it 'applies manifest twice with stderr check' do
      expect(apply_manifest(pp, expect_failures: true).first['result']['stderr']).to match("Duplicate key 'one' found with values 'foo' and bar'. Use 'force' attribute to merge keys.")
      expect(apply_manifest(pp, expect_failures: true).first['result']['stderr']).to match("Duplicate key 'one' found with values 'foo' and bar'. Use 'force' attribute to merge keys.")
