...
describe 'concat::fragment source' do
  ...
  describe 'when run should fail if no match on source.' do
    ...
    it 'applies the manifest with resource failures' do
      expect(apply_manifest(pp, expect_failures: true).first['result']['stderr']).to match(%r{Failed to generate additional resources using 'eval_generate'})
      expect(file("#{@basedir}/fail_no_source")).to be_file
    end
  end
end
